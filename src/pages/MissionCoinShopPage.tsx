...
const MissionCoinShopPage = () => {
  ...
  const { missionCoins, addMissionCoins } = useAppStore();
  ...
  const confirmPurchase = async () => {
    ...
    addMissionCoins(selectedPkg.amount);
    setPurchasing(false);
    toast.success(`미션 코인 ${selectedPkg.amount}개를 구매했습니다!`);
    setSelectedPkg(null);
  };
  return (
    <div className="h-full flex flex-col relative">
      ...
      <div className="relative z-10 px-5 pt-safe-top pb-4 flex items-center gap-3">
        ...
        <h1 className="text-lg font-bold text-white">미션 코인 상점</h1>
      ...
      <div className="relative z-10 px-5 mb-4">
        <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-semibold text-white">미션 코인이란?</span>
          </div>
          <p className="text-xs text-white/50 leading-relaxed">
            하루 무료 미션 재생성 3회를 모두 사용한 뒤, 추가로 미션을 재생성하거나 새로고침할 때 사용하는 코인입니다.
            미션 코인 1개당 미션 1회 재생성이 가능합니다.
          </p>
        </div>
      </div>
      {/* 패키지 목록 */}
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {pkg.popular && (
                            <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-cyan-500 text-white font-bold">인기</span>
                          )}
                          {pkg.bestValue && (
                            <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-yellow-500 text-white font-bold">최고 가성비</span>
                          )}
                          {discount > 0 && (
                            <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-red-500/80 text-white font-bold">-{discount}%</span>
                          )}
                        </div>
                    ...
                    <div className="text-right">
                      ...
                      {discount > 0 && (
                        <div className="text-[11px] text-white/30 line-through">₩{formatPrice(pkg.amount * 500)}</div>
                      )}
                    </div>
                  </div>
                </button>
        ...
        <AlertDialogContent className="bg-slate-900 border-white/10 max-w-[340px] rounded-2xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-white/60">
              {selectedPkg && (
                <>
                  미션 코인 <span className="text-cyan-300 font-bold">{selectedPkg.amount}개</span>를{' '}
          ...
          <AlertDialogFooter className="flex-row gap-2 mt-2">
            <AlertDialogCancel className="flex-1 m-0 bg-white/10 border-white/10 text-white hover:bg-white/15 hover:text-white">취소</AlertDialogCancel>
            <AlertDialogAction onClick={confirmPurchase} className="flex-1 m-0 bg-cyan-600 hover:bg-cyan-700 text-white border-none">
